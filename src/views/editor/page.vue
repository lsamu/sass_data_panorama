<template>
  <div class="pagelistwrap" :class="{ closed: thatData.pageState }">
    <a class="closebar" @click="thatData.pageState = !thatData.pageState"></a>
    <div class="pagelist">
      ...
      <div class="pagewrap">
        <ul>
          <li
            v-for="(page, index) in thatData.pageContainer"
            :key="index"
            :class="{ active: thatData.npage == index }"
          >
            <div class="picindex">
              <a
                class="picup iconfont icon-jiantoushangsheng"
                @click="up(index)"
              ></a>
              <p>{{ index + 1 }}</p>
              <a
                class="picdown iconfont icon-jiantouxiajiang"
                @click="down(index)"
              ></a>
            </div>
            <div class="picwrap" @click="setPage(index)">
              <div style="transform-origin: 0px 0px; transform: scale(0.22);">
                <div
                  style="width: 640px; height: 1018px;"
                  :style="{
                    background:
                ...
                ></div>
                <div
                  v-for="(element, key) in page.layer"
                  :key="key"
                  :type="element.type"
                  class="item_wrapper"
                  style="position: absolute; background-repeat: no-repeat; box-shadow: rgba(0, 0, 0, 0.5) 0px 0px 0px; overflow-wrap: break-word;"
                  :style="{
                    borderRadius: element.borderRadius + 'px',
                    borderColor: element.borderColor,
                    borderStyle: element.borderStyle,
                    color: element.color,
                    opacity: (100 - element.opacity) / 100,
                ...
                >
                  <div :class="element.type + '-element'">
                    <div
                      ...
                      style="background-position: 0px 0px; overflow-wrap: break-word;"
                      :style="{
                        borderRadius: element.borderRadius + 'px',
                        borderColor: element.borderColor,
                        borderStyle: element.borderStyle,
                        ...
                        backgroundImage: 'url(' + element.value + ')',
                        backgroundSize:
                          ...
                          <div
                            ...
                            :style="{
                              ...
                              fontFamily: element.fontFamily
                                ? element.fontFamily
                                : '',
                              fontWeight: element.fontBold ? 'bold' : '',
                    ...
                    <div
                      ...
                      style="box-shadow: black 0px 0px 0px; overflow-wrap: break-word;"
                      :style="{
                        width: element.width + 'px',
                        height: element.height + 'px',
                        lineHeight: element.height + 'px',
                    ...
                    >
                      <div
                        ...
                        @click.stop="playVideo(element.id)"
                      >
                        <video
                          :ref="'video-' + element.id"
                          :src="element.value"
                          :width="element.width"
                    ...
                    >
                      <div
                        class="link-button-render render"
                        :style="{
                          width: element.width + 'px',
                          height: element.height + 'px',
                        }"
                        ...
                        <div class="text1 link-button-render-text">
                          <a
                            ...
                            :style="{
                              ...
                              borderRadius: element.borderRadius + 'px',
                              textAlign: element.textAlign,
                              overflowWrap: element.overflowWrap,
                              width: element.width + 'px',
                              height: element.height + 'px',
                              lineHeight: element.height + 'px',
                              color: element.color,
...
export default defineComponent({
  setup(props, context) {
    ...
    const up = function(index: number) {
      if (index == 0) return;
      const tmpPage = thatData.pageList;
      const o = tmpPage[index - 1];
      tmpPage[index - 1] = tmpPage[index];
      tmpPage[index] = o;
    };

    const down = function(index: number) {
      if (index + 1 == thatData.pageList.length) return;
      const tmpPage = thatData.pageList;
      const o = tmpPage[index + 1];
      tmpPage[index + 1] = tmpPage[index];
      tmpPage[index] = o;
    ...
    const copyPage = function(index: number) {};
    const deletePage = function(index: number) {
      if (thatData.pageList.length == 1) {
        // this.$message({
        //   type: 'warning',
        //   message: '至少保留一页'
        // });
    ...
    const addPage = function() {
      //context.emit("addPage");
    };

    const playVideo = function(index: number) {};
...
.pagelist .pagewrap li.active .picwrap,
.pagelist .pagewrap li:hover .picwrap {
